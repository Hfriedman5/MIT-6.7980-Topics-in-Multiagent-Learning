...
- Label ratings as `Elo` in the FoW interface; use standard per-team Elo ratings with K=16 and the backend API identifier `elo`. Rank by Elo, with no uncertainty or lower-bound score. Select available pairs with weights proportional to the standard deviation of the Elo expected score, then play both colors. Every rated game counts across versions.
