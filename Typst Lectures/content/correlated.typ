// Port of Lectures/content/correlated.tex; Fall 2025 source.
#import "../meta/gabri_notes_bk.typ": *
#show: gabri_notes.with(lec_num: 3, date: [Tue, Sep 22, 2026], title: [Properties and relaxations of the Nash equilibrium], instructor: [Prof. Gabriele Farina])
...
#align(center)[
    ...
    #image("../figures/L04/game_table.svg")
  ]
...
#theorem[
Consider a two-player zero-sum game, that is, one for which $U_2 = - U_1$. Then, a strategy profile $(x^(*) \, y^(*)) in Delta (A_1) times Delta (A_2)$ is a Nash equilibrium if and only if it is a maxmin strategy, #emph[i.e.], if and only if
...
[
The connection between linear programming and two-player zero-sum games is bidirectional: as it turns out, #emph[solving linear programming] and #emph[finding a Nash equilibrium in a two-player zero-sum game] are #emph[computationally equivalent]. This means that #emph[any] linear programming problem (with arbitrary constraints, variables, etc.) can be efficiently converted into a two-player zero-sum game. This is less obvious than it may seem. For one, the strategy sets in games are probability simplexes, while linear programs might have arbitrary linear equality and inequality constraints. Furthermore, linear programs might be unbounded or unfeasible; yet, a Nash equilibrium of a game always exists. It would have been perfectly reasonably to believe that linear programming was a significantly more general tool than equilibrium solvers for two-player zero-sum games, and we know today that that belief would have been wrong. For more on this, see #citep(label("Brooks2023Oct"), label("vonStengel2023Jul"), label("adler2013equivalence")).
],
[
...
#quote(block: true)[
          « On October 3, 1947, I visited him (von Neumann) for the first time at the Institute for Advanced Study at Princeton. I remember trying to describe to von Neumann, as I would to an ordinary mortal, the Air Force problem. I began with the formulation of the linear programming model in terms of activities and items, etc. Von Neumann did something which I believe was uncharacteristic of him. “Get to the point,” he said impatiently. Having at times a somewhat low kindlingpoint, I said to myself “O.K., if he wants a quicky, then that's what he will get.” In under one minute I slapped the geometric and algebraic version of the problem on the blackboard. Von Neumann stood up and said “Oh that!” Then for the next hour and a half, he proceeded to give me a lecture on the mathematical theory of linear programs. At one point seeing me sitting there with my eyes popping and my mouth open (after I had searched the literature and found nothing), von Neumann said: “I don't want you to think I am pulling all this out of my sleeve at the spur of the moment like a magician. I have just recently completed a book with Oskar Morgenstern on the theory of games. What I am doing is conjecturing that the two problems are equivalent. The theory that I am outlining for your problem is an analogue to the one we have developed for games.” Thus I learned about Farkas' Lemma, and about duality for the first time.»
            ...
            (from #citet(label("Dantzig1982Apr")))
        ]
],
[
...
#strong[Topological properties]  It is important to realize that what #ref(label("thm:nash is mm")) is saying is that in two-player zero-sum games, each player can plan their own strategy #emph[independently]. #emph[Any] combination of maxmin strategies for the players forms an equilibrium. This is in contrast with the general case: there, in order to specify a Nash equilibrium we need to provide a tuple of strategies for all players. In two-player zero-sum games, instead, #emph[any product of maxmin strategies is an equilibrium]. We have just arrived at the following corollary.
...
In the general two-player case, often referred to as #emph[two-player general-sum games], many of the nice properties of the zero-sum case are lost.

#strong[Topological properties]  Perhaps the most striking is that not only the set of Nash equilibria is no longer guaranteed to be convex, but it is not even guaranteed to be contractible. We show this phenomenon with the next example.

#remark[Complex topology of Nash equilibria; Kohlberg-Mertens game #citep(label("kohlberg1986strategic"))][
...
For one, #emph[it is not at all guaranteed that the set is convex]. Even more, the set might be #emph[topologically complex], #emph[e.g.], exhibiting holes.  This phenomenon was already observed by #citet(label("kohlberg1986strategic")), who considered the two-player three-action game

#align(center)[
      #image("../figures/L03/km_game.svg", width: 118.75pt)
    ]

The figure on the right, similar to the one in #citep(label("Milionis2023Oct")), shows the projection of the set of all $0.27$-approximate Nash equilibria of this game, #emph[i.e.], all strategy profiles such that no player has a unilateral deviation that increases their utility by more than $0.27$.
][
#image("../figures/L03/kohlberg_mertens.svg", width: 175.392pt)
]
]

#strong[Computation]  In two-player general-sum games, computation of Nash equilibria is not a linear program. However, it is a #emph[linear complementarity problem] (LCP), a more general class of problems than linear feasibility programs, and which are written in the form
...
#strong[Analytic properties]  As a start, #emph[rational numbers might not be enough anymore] to store the probabilities of each player's actions at equilibrium.

#example[
In his original paper, Nash showed a three-player game with rational payoffs and with the property that #emph[all] Nash equilibria prescribe probabilities that are irrational numbers #citep(label("Nash51:NonCooperative")). Another simple example is also reported by #citet(label("nau2004geometry")), as follows: 

#align(center)[
    #image("../figures/L03/nash_irrational.svg")
  ]
...
$ (x_1^(*) \, y_1^(*) \, z_1^(*)) = (53 / 46 - sqrt(601) / 46 \, - 13 / 24 + sqrt(601) / 24 \, - 23 / 4 + sqrt(601) / 4) approx (0.619 \, 0.480 \, 0.379) . $
  ...
  From a computational point of view, this property raises the question of how a Nash equilibrium solver could even #emph[represent] such an output.
]
...
#remark[
The issues with irrational numbers do not stop at square roots. In fact, #emph[any polynomial root] might be required to represent a Nash equilibrium. This was shown by #citet(label("bubelis1979equilibria")), who showed how to construct games with arbitrary polynomial roots.

  Beyond the representation, the topology of Nash equilibria is also in general arbitrarily complex in three-player games. In particular, #citet(label("datta2003universality")) showed that for any real algebraic variety, one can come up with some three-player game whose set of Nash equilibria is isomorphic to that variety.
]

#strong[Computation]  On the computational side, the situation is even more dire. As a first consideration, because Nash equilibria might require irrational numbers, even the question of how to #emph[represent] the output equilibrium needs attention. In general, we cannot hope for an #emph[exact] value. However, even asking for a #emph[constant] approximation turns out to be hard. We will talk about this in more detail at the end of the course, where we relate the computation of (approximate) Nash equilibria to a complexity class called PPAD.

If one is willing to stomach a worst-case superpolynomial runtime, some methods exist. While the Lemke-Howson algorithm cannot be used beyond two-player games, other methods (such as #citep(label("Porter2008Jul"))) still apply.
...
Here, $u_i$ was defined as the expected payoff when all the players randomize #emph[independently].

The concept of #emph[coarse correlated equilibrium] is a relaxation of this definition. In a coarse correlated equilibrium, instead of asking for the players to pick #emph[independent] strategies, we allow coordination. In particular, we define the following.

#definition[Coarse correlated equilibrium #citep(label("moulin1978strategically"))][
...
Furthermore, expanding the expectation in inequality #ref(label("eq:cce"), supplement: none) defines a set of linear constraints
...
Hence, the set of CCEs is the intersection of a finite set of linear constraints, and so it is a convex polytope. Note that the number of constraints is polynomial in the game (#emph[i.e.], in the size of the payoff table), and so we can use linear programming to compute and even optimize over the set of CCEs in time polynomial in $\| A_1 \| times dots.h times \| A_n \|$.
...
#definition[Correlated equilibrium #citep(label("Aumann1974Mar"))][
A #emph[correlated equilibrium (CE)] is a correlated strategy $mu in Delta (A_1 times dots.h times A_n)$ such that
...
We can think of the correlation between the strategies of the players in a correlated or coarse correlated equilibrium as arising from some #emph[correlation device] in the game. This is a trusted mediator that can recommend but not enforce behavior. The distribution $mu$ from which the correlation device samples recommendations is public knowledge, but the players only get to observe the recommended action that was sampled for them. A correlated / coarse correlated equilibrium is then a distribution $mu$ such that no player can unilaterally deviate from the recommended action to improve their payoff.

The distinction between correlated and coarse correlated equilibrium is in when the players decide when to commit to the recommended action. In a coarse correlated equilibrium, the players commit to the recommended action #emph[before] the recommendation is made. In a correlated equilibrium, the players commit to the recommended action #emph[after] the recommendation is made.
