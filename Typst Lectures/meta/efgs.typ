  ...
  #cetz.canvas({
    ...
    let infoset(inodes, bend: 0mm, name: none, right: false) = {
      ...
      if name != none and infoset-labels {
        ...
        content(p)[#text(font: "New Computer Modern Sans 08", fill: blue)[#name]]
      }
    }
    for is in infosets.pos() {
      infoset(is.nodes, bend: is.bend, name: is.name, right: is.at("right", default: false))
    }
    ...
    for (key, pos) in nodes {
      if key != "/" {
        ...
        if show-label {
          outlined-content(c)[#text(font: "New Computer Modern Sans 08", action-name(action))]
        }
      }
    }
