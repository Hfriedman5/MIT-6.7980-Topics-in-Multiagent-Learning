...
#let citation_supplement_text(value) = {
  if value == none {
    ""
    ...
    if value == linebreak() {
      ...
      citation_supplement_text(value.text)
    } else if value.has("children") {
      value.children.map(citation_supplement_text).join("")
    } else if value.has("body") {
      citation_supplement_text(value.body)
    } else if value.has("child") {
      citation_supplement_text(value.child)
    } else {
      ""
    }
  } else if type(value) == array {
    value.map(citation_supplement_text).join("")
  } else {
    str(value)
  }
}
...
#let citation_label_text(key, cited_keys: (), ..supplement) = {
  let suffix = if supplement.pos().len() > 0 {
    ", " + citation_supplement_text(supplement.pos().first())
  } else {
    let named = supplement.named()
    if "supplement" in named {
      ", " + citation_supplement_text(named.at("supplement"))
    } else {
      ""
    }
  }
