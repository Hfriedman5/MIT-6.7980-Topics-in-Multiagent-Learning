...
#definition[
  In a symmetric game, a Nash equilibrium $x = \( x_1 \, dots.h \, x_n \)$ is called _symmetric_ if $x_1 = x_2 = dots.h = x_n$, i.e.~all players use the same mixed strategy.
]
...
#proof[
  ...
  #proof[
    ...
    #math.equation(block: true, numbering: "(1)", $tilde(x)_1^T R hat(y)_2 > hat(x)_1^T R hat(y)_2 .$.body)#label(
    ...
    Given Subclaim~#ref(<subclaim1>, supplement: none), we get a contradiction to our assumption that $\( \[ x_1 \; y_1 \] \, \[ x_2 \; y_2 \] \)$ is a Nash equilibrium of $cal(G)_2$.
  ]
]
...
#definition[
  Action $i$ is _represented_ at a vertex $z$ of the polytope if at least one of the following inequalities is tight:
...
- *Step $0$:* Start at  vertex $v_0 := \( 0 \, 0 \, dots.h \, 0 \)$.
- _Comment:_ By non-degeneracy, there are exactly $n$ edges of the polytope adjacent to $v_0$. Each of these edges corresponds to un-tightening one of the $z_i gt.eq 0$ inequalities which are tight at $v_0$.
...
- *Step 1:* Go to the other endpoint of this edge. If the obtained vertex $v_1$ is a democracy, then a Nash equilibrium has been found because $v_1 eq.not 0$.
...
- _Comment:_ For the next step, we will un-tighten one of the two inequalities that are tight for $j_1$. If we un-tighten $R_(j_1) z lt.eq 1$, this would define the same edge $\( v_0 v_1 \)$ that brought us to $v_1$. To make progress we will un-tighten instead the other inequality representing action $j_1$.
...
- *Step 2:* Go to vertex $v_2$. If $v_2$ is a democracy, then stop.
  ...
  _Comment:_ It will be shown (in the correctness analysis below) that it must be that $v_2 eq.not 0$, and hence if $v_2$ is a democracy then $v_2 \/ norm(v_2)_1$ is a symmetric Nash equilibrium.
...
- *Step $t$:* At the generic step $t$ of the algorithm, the algorithm arrives at vertex $v_t$ and performs the following case analysis:
  ...
  - if $v_t$ is a democracy, stop. _Comment:_ It will be shown that it must be that $v_t eq.not 0$.
