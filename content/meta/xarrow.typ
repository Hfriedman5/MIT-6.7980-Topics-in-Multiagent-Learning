    ...
    math.attach(
      math.limits({
        if repeated_width < (end - begin) * sym_w {
          ...
          block(width: begin * sym_w, height: sym_h, clip: true, place(dx: (100% - begin) / 2 * sym_w, sym))
          ...
          fit_width(
            ...
            block(
              height: sym_h,
              width: (end - begin) * sym_w,
              clip: true,
              place(dx: (100% - begin - end) / 2 * sym_w, sym),
            ),
          )

          block(width: (100% - end) * sym_w, height: sym_h, clip: true, place(dx: -end / 2 * sym_w, sym))
        }
      }),
      t: top_content,
      b: bottom_content,
