...
#show: gabri_notes.with(lec_num: 7, date: [Tue, Oct 6, 2026], title: [Foundations of extensive-form games], instructor: [Prof. Gabriele Farina (#raw("gfarina@mit.edu"))])
...
#align(center)[
    #image("../figures/L10/variations.svg", width: 100.0%)
  ]
]
...
As an example, consider Player 1 in Kuhn poker #ref(label("ex:kuhn")). From the player's point of view, playing the game could be summarized as follows: 
...
#image("../figures/L10/kuhn_tfdp.png", width: 100.0%)
]#label("fig:kuhn-tfdp")
]
...
#example[
Consider Kuhn poker from the point of view of Player~1 (#ref(label("fig:kuhn-tfdp"))). 
...
Consider the following small game tree.
  ...
  [#image("../figures/L10/small_efg.svg", width: 210pt)],
  side: right,
  text-width: 65%,
)
]#label("ex:small-efg")

Traces implicitly encode a notion of partial chronological ordering between information sets, of which the player has recall---see #ref(label("def:perfect-recall")). Hence, for the TFDP of Player~$i$ to be an accurate representation of the decision process the player faces while playing the game, it is necessary that #emph[traces of the information sets are the same in the game tree and in the TFDP]. In other words, we require that decision points in the TFDP be structured so as to satisfy that the trace of any information set $I$ matches the sequence of information sets and actions encountered from the root of the TFDP to decision node $I$.

#definition[Tree-form decision process][
Fix the game tree of an $n$-player imperfect information game, and a player $i in \[ n \]$. A #emph[tree-form decision process (TFDP) for Player~$i$] is a directed rooted tree made of #emph[decision], #emph[observation], and #emph[terminal nodes], satisfying the following properties. 
...
[
The set of actions available at each decision node $j = I in cal(I)_i$ (#emph[i.e.], the set of outgoing edges from the decision node) is equal to the set of actions $A_I$ available at any history $h in I$ in the game tree.
],
[
...
As a remark, #ref(label("def:tfdp")) leaves the labeling and structure of observation nodes unspecified. In fact, a player might have multiple TFDPs that satisfy the definition, and differ in how the observation nodes are placed. We illustrate this in the next example.

#example[
The following are both valid TFDPs capturing Player~1's decision process when playing Kuhn poker (#ref(label("fig:kuhn-tfdp"))).

#align(center)[
    #image("../figures/L10/kuhn_alternatives-transparent.png", width: 100.0%)
  ]
]

#example[
A valid TFDP representing the decision process of Player 1 in the small game of #ref(label("ex:small-efg")) is shown below.

#align(center)[
    #image("../figures/L10/small_tfdp.png")
  ]
]
...
#block(sticky: true)[#paragraph-marker() #strong[Decision and observation nodes, transition function:]]
...
#block(sticky: true)[#paragraph-marker() #strong[Sequences:]]
...
#example[
In the small game of #ref(label("ex:small-efg")), a mixed strategy for Player 1 is a probability distribution over the following 7 strategies $pi_1 \, dots.h \, pi_7$.

#align(center)[
    #image("../figures/L10/nf_strategies.svg", width: 100.0%)
  ]

  These strategies are called the #emph[reduced]#footnote[The term “reduced” refers to the fact that no actions are specified for parts of the games not reached due to decisions of the player in higher parts of the game tree.] #emph[normal-form plans] of the player.
]
...
#example[
Consider the tree-form decision process faced by Player 1 in the small game of #ref(label("ex:small-efg")). The decision process has four decision nodes $J = {sans(A) \, sans(B) \, sans(C) \, sans(D)}$ and nine sequences including the empty sequence $∅$. For decision node D, the parent sequence is $p_(sans(D)) = sans(A 2)$; for $sans(B)$ and $sans(C)$ it is $p_(sans(B)) = p_(sans(C)) = sans(A 1)$; for $sans(A)$ it is the empty sequence $p_(sans(A)) = ∅$. The constraints that define the sequence-form polytope #ref(label("def:sf")), besides nonnegativity, are
...
$ cases(x_∅ = 1, x_(sans(A 1)) + x_(sans(A 2)) = x_∅, x_(sans(B 3)) + x_(sans(B 4)) = x_(sans(A 1)), x_(sans(C 5)) + x_(sans(C 6)) = x_(sans(A 1)), x_(sans(D 7)) + x_(sans(D 8)) + x_(sans(D 9)) = x_(sans(A 2)) .) $
  ...
  [#image("../figures/L10/small_tfdp.png", width: 170pt)],
  side: right,
  text-width: 65%,
)
]

#strong[Pros]  This representation of extensive-form games is convex. Hence, we can use all the convex optimization tools we have seen so far.
