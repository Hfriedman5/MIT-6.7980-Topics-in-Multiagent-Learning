...
$ Phi := {P = (p_1 | dots.c | p_n): p_1, dots.c, p_n in Delta^n}. $
...
- To compute the `NextStrategy()` of the swap regret minimizer, we first call the `NextStrategy()` of each $cR_i$ to obtain a distribution $p_i^((t)) in Delta^n$. We will then assemble all $p_i^((t))$ into a stochastic matrix $P^((t))$, and will output a fixed point
...
#figure(caption: [Blum-Mansour's swap regret minimization algorithm for $Delta^n$.])[
  #image("../assets/L08-blum_mansour.svg", width: 100%, alt: "Blum-Mansour construction: an external regret minimizer per action and a fixed-point oracle.")
] <fig-bm>
...
#proof[
  ...
  $
    "Reg"_i^((T)) = max_(tilde(p)_i in Delta^n)
    sum_(t=1)^T (v^((t))(tilde(p)_i) - v^((t))\(p_i^((t))\)).
  $ <eq-def-Rv>
  #set math.equation(numbering: none)
  Pick any $hat(P) = (hat(p)_1 | ... | hat(p)_n) in Phi$. Then, the swap regret cumulated compared to always transforming strategies according to $hat(P)$ is, by definition,
  $
...
#figure(caption: [Gordon-Greenwald-Marks's reduction from $Phi$-regret minimization to external regret minimization for $Phi$ plus a fixed point oracle for $Phi$.])[
  #image("../assets/L08-gordon.svg", width: 100%, alt: "General Phi-regret reduction: utility construction, external regret minimization over transformations, and fixed-point computation.")
] <fig-ggm>
