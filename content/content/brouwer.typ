...
#show: gabri_notes.with(lec_num: 2, date: [Thu, Sep 17, 2026], title: [Existence Proofs: Nash, Brouwer, and Sperner], instructor: [Prof. Constantinos Daskalakis])
...
#image("../figures/L17/sperner_example.svg", width: 138.459pt)
]#label("fig:sperner coloring")
]

 Sperner's lemma applies to a $3$-colored $N times N$ grids of points. There are high-dimensional analogues of the lemma, but we will stick to $2$ dimensions in this lectures. For the lemma to apply, the rules are simple. Each point is colored with one of three colors: red, blue, or yellow. The coloring must, however, satisfy the following #emph[boundary] conditions:
...
#image("../figures/L17/sperner_triangulation.svg")
]
]
...
#wrapped-figure(side: left, text-width: 67%)[
...
#image("../figures/L03/color_wheel.svg", width: 92.774pt)
]
...
#align(center)[
    #image("../figures/L17/example_games.svg", width: 100.0%)
  ]
]
...
#math.equation(block: true, numbering: "(1)", $forall epsilon.alt > 0 \, exists delta (epsilon.alt) & > 0 :\
 & ∥z - w∥_oo < delta (epsilon.alt) quad arrow.r.double.long quad ∥f \( z \) - f \( w \)∥_oo < epsilon.alt .$.body)#label("eq: uniform continuity")
...
#image("../figures/L17/sperner_padded.svg")
]
...
#image("../figures/L17/sperner_paths.svg", width: 155.78pt)
]
