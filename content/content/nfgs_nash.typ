...
#list(
[
The set of players $\[ n \] = {1 \, dots.h \, n}$.
],
[
For each player $i in \[ n \]$, a finite set of actions $A_i$.
],
[
For each player $i in \[ n \]$, the payoff function $u_i : A_1 times dots.h.c times A_n arrow.r bb(R)$.
]
)
...
#image("../figures/L01/prisoner_dilemma.svg", width: 103.498pt)
]
]

#strong[Strategies]  A #emph[randomized strategy] (also known as #emph[mixed strategy]) for a generic player $i in \[ n \]$ is a distribution over the set of actions. We can represent such an object as a vector $x_i in Delta (A_i)$, that is, such that $x_i gt.eq 0$ and $sum_(a_i in A_i) x_(i \, a_i) = 1$. To lighten the notational burden, we will write the expected utility when all players play according to strategies $x_1 \, dots.h \, x_n$ reusing the same letter $u_i$ as the payoff, #emph[i.e.],
...
The question of what constitutes rational play for players can get complicated depending on the game. But, in some lucky cases, like the prisoner's dilemma game above, it turns out that some actions are just #emph[better] than others, #emph[no matter what the other players do]. In such cases, we say that a player has a #emph[dominant strategy]. In the case above, both Player 1 and Player 2 have a dominant strategy to confess$.$ In this case, we expect that the players will play their dominant strategy, and this is called a #emph[dominant-strategy equilibrium].
...
#math.equation(block: true, numbering: (..nums) => "(Dominant-strategy eq.)", $forall i in \[ n \] \, x'_i in Delta (A_i) \, x'_(- i) in Delta (A_(- i)) \, quad u_i (x'_i \, x'_(- i)) lt.eq u_i (x_i \, x'_(- i)) .$.body)
...
#example[
The only Nash equilibrium in the game of rock-paper-scissors is for all players to play the uniform strategy. This shows that in some games, no Nash equilibrium exists in pure (#emph[i.e.], non-randomizing) strategies.
]
...
#align(center)[
    #image("../figures/L01/theater_football.svg", width: 100.223pt)
  ]
...
#list(
[
The idea that Nash equilibria can be thought of as fixed points should feel extremely natural. By definition, an equilibrium is a situation where nobody has an incentive to deviate---that is, no player has a more profitable strategy given the strategies of the other players. Hence, if we are able to introduce a function that maps a strategy profile to a profile of “improved strategies”, then a fixed point of such a function would be a Nash equilibrium. We could then use one of the many theorems in analysis that guarantee existence of fixed points. Of course, the devil is in the details: #emph[how to handle multiple profitable responses? how to ensure continuity of the deviation function?] These are the questions that Nash had to answer.
],
[
The idea of viewing Nash equilibria as fixed points is not just natural, but also #emph[the only possible]. We will make this formal towards the end of this course, when we discuss the #emph[computational complexity] of Nash equilibria. In particular, we will show that the computation of fixed points of continuous functions and computation of Nash equilibria are computationally equivalent, in the sense that each problem can be reduced to the other in polynomial time.
]
)
...
#strong[Regret]  To formally define the Nash improvement function, we first introduce a simple quantity called #emph[regret], which will be a staple of this course. The #emph[regret] that Player $i$ experiences with respect to action $a_i in A_i$ is the difference between the payoff that Player $i$ would have obtained by playing $a_i$, and the payoff that Player $i$ actually obtained:

$ r_(i \, a_i) (x_1 \, dots.h \, x_n) colon.eq u_i (a_i \, x_(- i)) - u_i (x_1 \, dots.h \, x_n) . $

#strong[The Nash improvement function]  The idea is simple: if an action $a_i$ has very large regret, then the current strategy profile cannot be an equilibrium, because Player $i$ would want to increase the probability of playing $a_i$. Thus, an “improved” strategy for Player $i$ should move more probability mass to $a_i$. We need to handle two complications: (1) if multiple actions have positive regret, how should we prioritize adding mass to those? and (2) for those actions whose regret is negative (that is, “bad” actions), should we forcefully decrease the mass?
...
#definition[Nash improvement function #citep(label("Nash51:NonCooperative"))][
Let $x_1 in Delta (A_1) \, dots.h \, x_n in Delta (A_n)$ be arbitrary strategies. The #emph[Nash improvement function] $phi : Delta (A_1) times dots.h.c times Delta (A_n) arrow.r Delta (A_1) times dots.h.c times Delta (A_n)$ is the map
...
#align(center)[
    #image("../figures/L01/nash_plots.svg", width: 100.0%)
  ]
...
#image("../figures/L01/color_wheel.svg", width: 82.527pt)
]
]
